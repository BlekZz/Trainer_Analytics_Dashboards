...
`Agent/` 目錄存放本專案所有 active agent 的 `.md` 備份，隨 repo 一起 commit。

**目的：** clone 此 repo 的協作者可直接將檔案複製到本機的 `~/.claude/agents/` 目錄，
無需從 Claude marketplace 重新安裝，即可調用完整的工作流 agent 組合。

**使用方式：**
```
cp Agent/*.md ~/.claude/agents/
```
複製後在此專案執行 `/agent-bench-setup` → Re-scan，CLAUDE.md 的 Active Agents 表格即自動生效。

**維護規則：** 若 active agent 的內容有更新，同步更新 `Agent/` 裡對應的 `.md` 檔案並 commit。
